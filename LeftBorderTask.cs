...
public class LeftBorderTask
{
    ...
    public static int GetLeftBorderIndex(IReadOnlyList<string> phrases, string prefix, int left, int right)
    {
        ...
        int comparison = string.Compare(phrases[mid], prefix, StringComparison.InvariantCultureIgnoreCase);
        bool startsWith = phrases[mid].StartsWith(prefix, StringComparison.InvariantCultureIgnoreCase);

        if (comparison < 0 && !startsWith)
